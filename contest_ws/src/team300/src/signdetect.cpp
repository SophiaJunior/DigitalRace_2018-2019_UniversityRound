...
SignType SignDetector::IpRecognize(const Mat &binImg)
{
	//cout << "IP" << endl;
...
SignType SignDetector::MlRecognize(const Mat &grayImg)
{
	//cout << "ML" << endl;
...
SignType SignDetector::Recognize(const Mat &binImg, const Mat &grayImg)
{
	return (USE_ML ? MlRecognize(grayImg) : IpRecognize(binImg));
}

void SignDetector::Update(const Mat &colorImg)
{
	// reset
	m_rect = RECT_ZERO;
	m_type = SignType::NONE;
	...
	//imshow(Window_SignBinary, binImg);

	if (this->Detect(binImg))
		this->Recognize(binImg, grayImg);

	Mat signDebug = colorImg.clone();

	if (m_type != SignType::NONE)
	{
		//cout << (m_type == SignType::TURN_LEFT ? "Sign left" : "Sign right") << endl;

		rectangle(signDebug, m_rect, Scalar(0, 0, 255));
		putText(signDebug,
				(m_type == SignType::TURN_LEFT ? "Sign left" : "Sign right"),
				Point(m_rect.x, m_rect.y),
				CV_FONT_HERSHEY_COMPLEX_SMALL,
				0.5,
				Scalar(0, 0, 255));
	}

	imshow(Window_DebugSign, signDebug);
}